    ...
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.dataset_attr.history and isinstance(example[self.dataset_attr.history], list):
            for old_prompt, old_response in example[self.dataset_attr.history]:
                prompt.append({"role": Role.USER.value, "content": old_prompt or ""})
                prompt.append({"role": Role.ASSISTANT.value, "content": old_response or ""})

        query = []
        if self.dataset_attr.prompt and example[self.dataset_attr.prompt] is not None:
            query.append(example[self.dataset_attr.prompt] or "")

        if self.dataset_attr.query and example[self.dataset_attr.query] is not None:
            query.append(example[self.dataset_attr.query] or "")

        prompt.append({"role": Role.USER.value, "content": "\n".join(query)})  # "prompt\nquery"

        if self.dataset_attr.kto_tag and isinstance(example[self.dataset_attr.kto_tag], bool):  # kto example
            response = [{"role": Role.ASSISTANT.value, "content": example[self.dataset_attr.response] or ""}]
        ...
        output = {
            "_prompt": prompt,
            "_response": response,
            "_system": (example[self.dataset_attr.system] or "") if self.dataset_attr.system else "",
            "_tools": (example[self.dataset_attr.tools] or "") if self.dataset_attr.tools else "",
...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        messages = example[self.dataset_attr.messages] or []
        if not isinstance(messages, list):
            logger.warning_rank0(f"Invalid messages type in {example}.")
            messages = []
        if (
            ...
            system = messages[0][self.dataset_attr.content_tag] or ""
            messages = messages[1:]
        else:
            system = (example[self.dataset_attr.system] or "") if self.dataset_attr.system else ""

        aligned_messages = []
        broken_data = False
        for turn_idx, message in enumerate(messages):
            if not isinstance(message, dict):
                logger.warning_rank0(f"Invalid message type in {messages}.")
                broken_data = True
                break
            ...
            aligned_messages.append(
                {
                    ...
                    "content": message[self.dataset_attr.content_tag] or "",
                }
            )
    ...
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        for turn_idx, message in enumerate(messages):
            ...
            if role in [self.dataset_attr.assistant_tag, self.dataset_attr.function_tag]:
                if isinstance(message.get("tool_calls", None), list) and len(message["tool_calls"]) > 0:
            ...
            if role == self.dataset_attr.observation_tag:
                tool_responses.append(content or "")
                continue
