...
from utils import load_config, load_prompts, format_prompt_strict, get_logit_bias_for_markers, get_random_logit_bias, has_leakage
...
def main():
    ...
    print(f"[{args.model_alias}] Starting Random-Direction Control Experiment...")
    ...
    for gamma in config['gamma_grid']:
        if gamma == 0:
            # Baseline is same for both
            modes = ["real"] 
        else:
            modes = ["real", "random"]
            
        for mode in modes:
            print(f"Running Gamma={gamma} Mode={mode}...")
            
            if mode == "real":
                logit_bias = get_logit_bias_for_markers(model_path, markers, gamma, scale=5.0)
            else:
                logit_bias = get_random_logit_bias(model_path, markers, gamma, seed=seed, scale=5.0)
            ...
            print(f"  Gamma={gamma} Mode={mode}: Leakage={leak_rate:.2f}")
    ...
    # Save
    csv_path = os.path.join(output_dir, f"random_control_{args.model_alias}.csv")
